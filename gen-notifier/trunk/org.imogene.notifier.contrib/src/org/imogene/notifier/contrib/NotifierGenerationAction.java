package org.imogene.notifier.contrib;
...
public class NotifierGenerationAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		try {
			...
			wizard.setWindowTitle("Generation of a notifier project");
			wizard.setSelectedProject(mSelectedProject);
			wizard.setArchive(FileLocator.openStream(Activator.getDefault().getBundle(), new Path("template-site/template.zip"), false));
			wizard.setDefinition(FileLocator.openStream(Activator.getDefault().getBundle(), new Path("template-site/templates.xml"), false));
			wizard.setWorkflow("workflow/generatorNotif.mwe");
