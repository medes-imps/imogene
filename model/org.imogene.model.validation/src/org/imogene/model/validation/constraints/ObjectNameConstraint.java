...
public class ObjectNameConstraint extends AbstractMedanyModelConstraint {
	...
	private static String EMPTY_PROJECT_NAME = "The name of the project is not valid and should only contain "
		+ reg_ex;
			...
			if (ctx.getTarget() instanceof Project) {
				...
				if (fieldName != null)
					if (fieldName.isEmpty())
						return ctx.createFailureStatus(new Object[] { formatMessage(EMPTY_FIELD_NAME, (CardEntity) ctx.getTarget()) });
					else if (!fieldName.matches(reg_ex))
						return ctx.createFailureStatus(new Object[] { formatMessage(BAD_FIELD_NAME, (FieldEntity) ctx.getTarget()) });
	...
	private String getErrorMessageEnumValue(String enumName) {
		return new String("The name of the name enum value \"" + enumName
				+ "\" should contains only " + reg_ex + ".");
	}

	/** get error message for role */
	private String getErrorMessageRole(String roleName) {
		return new String("the name of the role \"" + roleName
				+ "\" should contains only " + reg_ex + ".");
	}
}
