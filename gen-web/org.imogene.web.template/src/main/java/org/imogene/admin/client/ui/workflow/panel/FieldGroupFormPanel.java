...
import org.imogene.admin.client.ui.workflow.FieldGroupEditorWorkflow;
...
/**
 * Panel that contains the FieldGroup Workflow
 * @author Medes-IMPS
 */
public class FieldGroupFormPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, FieldGroupFormPanel> {
	}

	@UiField(provided = true)
	WrapperPanel wrapperPanel;
	@UiField(provided = true)
	FieldGroupEditorWorkflow editorWorkflow;
	 ...
	 */
	public FieldGroupFormPanel(AdminRequestFactory requestFactory, String entityId, RelationPopupPanel parent, String initField, ImogBeanRenderer renderer) {
		...
		if (entityId != null) {
			if (parent == null)
				editorWorkflow = new FieldGroupEditorWorkflow(requestFactory, entityId, titleContainer, renderer);
			else {
				editorWorkflow = new FieldGroupEditorWorkflow(requestFactory, entityId, titleContainer, parent, initField, renderer);
			}
		} else {
			if (parent == null) {
				editorWorkflow = new FieldGroupEditorWorkflow(requestFactory, titleContainer);
			} else {
				editorWorkflow = new FieldGroupEditorWorkflow(requestFactory, titleContainer, parent, initField);
			}
		}
	 ...
	 * @param entityId the id of the entity to be managed by the workflow. null if an entity is being created
	 */
	public FieldGroupFormPanel(AdminRequestFactory requestFactory, String entityId, ImogBeanRenderer renderer) {
