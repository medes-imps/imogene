...
import org.imogene.admin.client.ui.workflow.FieldGroupProfileEditorWorkflow;
...
/**
 * Panel that contains the FieldGroupProfile Workflow
 * 
 * @author Medes-IMPS
 */
public class FieldGroupProfileFormPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, FieldGroupProfileFormPanel> {
	}

	@UiField(provided = true)
	WrapperPanel wrapperPanel;
	@UiField(provided = true)
	FieldGroupProfileEditorWorkflow editorWorkflow;
	 ...
	 *            otherwise
	 */
	public FieldGroupProfileFormPanel(AdminRequestFactory requestFactory, String entityId, RelationPopupPanel parent,
			String initField) {
		...
		if (entityId != null) {
			if (parent == null)
				editorWorkflow = new FieldGroupProfileEditorWorkflow(requestFactory, entityId, titleContainer);
			else {
				editorWorkflow = new FieldGroupProfileEditorWorkflow(requestFactory, entityId, titleContainer, parent,
						initField);
			}
		} else {
			if (parent == null) {
				editorWorkflow = new FieldGroupProfileEditorWorkflow(requestFactory, titleContainer);
			} else {
				editorWorkflow = new FieldGroupProfileEditorWorkflow(requestFactory, titleContainer, parent, initField);
			}
		}
	 ...
	 * @param entityId the id of the entity to be managed by the workflow. null if an entity is being created
	 */
	public FieldGroupProfileFormPanel(AdminRequestFactory requestFactory, String entityId) {
	...
	/**
	 * Setter to inject a Profile value into the workflow and the editor
	 * 
	 * @param value the value to be injected
	 * @param isLocked true if relation field shall be locked (non editable)
	 */
	public void setProfile(ProfileProxy value, boolean isLocked) {
		editorWorkflow.setProfile(value, isLocked);
	}
	...
	public void setProfile(ProfileProxy value) {
		setProfile(value, false);
	}

	/**
	 * Setter to inject a FieldGroup value into the workflow and the editor
	 * 
	 * @param value the value to be injected
	 * @param isLocked true if relation field shall be locked (non editable)
	 */
	public void setFieldGroup(FieldGroupProxy value, boolean isLocked) {
		editorWorkflow.setFieldGroup(value, isLocked);
	}
