...
import org.imogene.admin.client.ui.table.EntityProfileDynaTable;
...
/**
 * Panel that contains the EntityProfile Dynatable
 * 
 * @author Medes-IMPS
 */
public class EntityProfileListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, EntityProfileListPanel> {
	}
	...
	@UiField(provided = true)
	EntityProfileDynaTable listComposite;
	 ...
	 */
	public EntityProfileListPanel(AdminRequestFactory requestFactory, String searchText) {
		...
		wrapperPanel.setTitle(AdminNLS.constants().entityProfile_table_text());

		/* data provider */
		EntityProfileDataProvider provider = new EntityProfileDataProvider(requestFactory);
		...
		if (ProfileUtil.isAdmin())
			listComposite = new EntityProfileDynaTable(requestFactory, provider, true);
		else
			listComposite = new EntityProfileDynaTable(requestFactory, provider, false);
	 ...
	 * @param requestFactory
	 */
	public EntityProfileListPanel(AdminRequestFactory requestFactory) {
		this(requestFactory, null);
	}

	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists EntityProfile entries
	 * 
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(EntityProfileDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists EntityProfile entries
	 */
