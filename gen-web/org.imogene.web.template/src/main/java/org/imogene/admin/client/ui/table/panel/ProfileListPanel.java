...
import org.imogene.admin.client.ui.table.ProfileDynaTable;
...
/**
 * Panel that contains the Profile Dynatable
 * 
 * @author Medes-IMPS
 */
public class ProfileListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ProfileListPanel> {
	}
	...
	@UiField(provided = true)
	ProfileDynaTable listComposite;
	 ...
	 */
	public ProfileListPanel(AdminRequestFactory requestFactory, String searchText) {
		...
		wrapperPanel.setTitle(AdminNLS.constants().profile_table_text());

		/* data provider */
		ProfileDataProvider provider = new ProfileDataProvider(requestFactory);
		...
		if (ProfileUtil.isAdmin())
			listComposite = new ProfileDynaTable(requestFactory, provider, true);
		else
			listComposite = new ProfileDynaTable(requestFactory, provider, false);
	 ...
	 * @param requestFactory
	 */
	public ProfileListPanel(AdminRequestFactory requestFactory) {
		this(requestFactory, null);
	}

	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Profile entries
	 * 
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(ProfileDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Profile entries
	 */
