 ...
 * @author MEDES-IMPS
 */
public class ImogBooleanBox extends Composite implements ImogField<Boolean>,
		LeafValueEditor<Boolean>, HasEditorErrors<Boolean> {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ImogBooleanBox> {
	}
	...
	@UiField
	@Ignore
	ImogBooleanAsRadio booleanBox;

	public ImogBooleanBox() {
		...
		if(eventBus!=null) {
			booleanBox.addValueChangeHandler(new ValueChangeHandler<Boolean>() {			
				...
				public void onValueChange(ValueChangeEvent<Boolean> arg0) {
					eventBus.fireEvent(new FieldValueChangeEvent(ImogBooleanBox.this));
				}
			});		
		}
	}
