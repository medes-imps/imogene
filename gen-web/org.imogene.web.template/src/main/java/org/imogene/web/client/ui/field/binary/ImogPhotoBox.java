 ...
 * @author Medes-IMPS
 */
public class ImogPhotoBox extends Composite implements ImogUploader,
		ImogField<BinaryProxy>, LeafValueEditor<BinaryProxy>, HasEditorErrors<BinaryProxy> {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ImogPhotoBox> {
	}
	
	private static String nophoto_url = GWT.getModuleBaseURL() + "images/no_photo.png";
	...
	public ImogPhotoBox(ImogRequestFactory requestFactory) {
		this.requestFactory = requestFactory;
		thumbnail = new Image(nophoto_url);
		uploader = new ImogBinaryUploader(thumbnail, requestFactory);
	...
	public void setValue(BinaryProxy value) {
		...
		if (value!=null && value.getId()!=null) {
			setThumbnail(value.getId());
			setDownloadLink(value.getId());
			if (value != thisValue)
