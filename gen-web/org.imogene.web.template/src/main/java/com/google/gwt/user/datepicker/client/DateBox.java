...
public class DateBox extends Composite implements HasEnabled, HasValue<Date>,
	...
	private class DateBoxHandler implements ValueChangeHandler<Date>,
		...
		public void onBlur(BlurEvent event) {
			if (enabled && isDatePickerShowing() == false) {
		...
		public void onClose(CloseEvent<PopupPanel> event) {
			...
			if (enabled && allowDPShow) {
				updateDateFromTextBox();
			}
		}

		public void onFocus(FocusEvent event) {
			if (enabled && allowDPShow && isDatePickerShowing() == false) {
				showDatePicker();
			}
		}
	...
	private static final DefaultFormat DEFAULT_FORMAT = GWT
			.create(DefaultFormat.class);
	...
	private DateBoxHandler handler;
	...
	/**
	 * Sets whether or not the date box will fire {@code ValueChangeEvents} with
	 * a date value of {@code null} for invalid or empty string values.
	 */
