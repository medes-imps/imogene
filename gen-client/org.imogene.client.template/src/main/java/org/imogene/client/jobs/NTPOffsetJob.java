...
public class NTPOffsetJob extends PeriodicalJob {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		try {
			...
			time.computeDetails();
			client.close();
			Activator.getDefault().getPreferenceStore().setValue(ISyncConstants.NTP_OFFSET, time.getOffset());
			return Status.OK_STATUS;
		} catch (Exception e) {
			return new Status(IStatus.ERROR, Activator.PLUGIN_ID, Messages.ntp_error, e);
		} finally {
			client.close();
			schedule(false, rate);
		}
